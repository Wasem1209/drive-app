...
export default function ConnectWallet({ onConnected }) {
	...
	const [connected, setConnected] = useState(null); // { key, label, api, address }
	...
	const attemptConnect = async (key) => {
		setError('');
		setConnecting(true);
		try {
			const api = await enableWallet(key);
			const addresses = await getUsedAddresses(api);
			const address = addresses[0] || ''; // blockchain dev can replace logic
			const walletMeta = available.find(w => w.key === key);
			const conn = { key, label: walletMeta?.label || key, api, address };
			setConnected(conn);
	...
	const renderChooser = () => (
		<div style={{ position: 'absolute', top: '100%', left: 0, marginTop: 8, background: '#0f172a', padding: '10px 12px', borderRadius: 12, width: 220, boxShadow: '0 6px 16px rgba(0,0,0,0.35)', zIndex: 20 }}>
			...
			{available.map(w => (
				<button key={w.key} disabled={connecting} onClick={() => attemptConnect(w.key)} style={{ display: 'flex', width: '100%', alignItems: 'center', gap: 8, background: '#1e293b', color: '#fff', border: '1px solid #334155', borderRadius: 8, padding: '6px 8px', marginBottom: 6, cursor: 'pointer' }}>
